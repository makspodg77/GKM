...
const StopGroup = () => {
  ...
  const Sort = (data: any) => {
    data = data.sort(
      (
        a: { departure_time: string; departure_id: number },
        b: { departure_time: string; departure_id: number }
      ) => {
        ...
        <div className="StopGroup">
          ...
          {Object.keys(stopGroupF).map((key, index) => (
            <div key={key} className="stop">
              ...
              <div>
                {stopGroupF[key].map((element) =>
                  FormatDepartureTime(element.departure_time) ? (
                    <div className="route">
                      <div>{element.line_name}</div>
                      <div>
                        <Link
                          to={`/rozklad-jazdy-wedlug-linii/${element.line_name}/${element.stop_id}/${element.route_number}`}
                        >
                          {element.last_stop_name}
                        </Link>
                      </div>
