...
const LineRoute = () => {
    const { timetableId, stopId } = useParams<{ timetableId: string, stopId: string }>();
                    ...
                    {route.map((stop: any) => {
                        ...
                        return (
                            <div key={stop.stop_id}>
                                ...
                                <div><Link to={`/zespol-przystankowy/${stop.stop_id}`}><img src={arrowFart} width="100%" /></Link></div>
                                <div style={!flag ? {opacity: '0.5'} : {}}><Link to={`/rozklad-jazdy-wedlug-linii/${stop.line_name}/${stop.stop_id}/${stop.route_number}`}>{stop.stop_name} ({stop.stop_id})</Link></div>
                                <div>{sum == 0 ? stopId == stop.stop_id ? '↓' : '' : sum + ' min'}</div>
