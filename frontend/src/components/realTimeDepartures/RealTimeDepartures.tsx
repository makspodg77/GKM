...
const RealTimeDepartures = () => {
    ...
    useEffect(() => {
        setLoading(true);
        service.getStopTimetable(Number(stopId)).then((data) => {
            if (data) {
            ...
            {loading ? <div>Loading...</div> : (
                <>
                    <div className="header">
                        <h1></h1>
                        <h1>{stopName} ({stopId})</h1>
                        <h2>{hoursNow}:{Number(minutesNow) < 10 ? "0" + minutesNow : minutesNow}</h2>
                    </div>
                    <div>
                        {departures.slice(0, (departures.length > 7 ? 7 : departures.length)).map((departure, index) => (
