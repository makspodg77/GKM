...
const MiniRealTimeDepartures = ({ id }: MiniRealTimeDeparturesInterface) => {
    ...
    const [areRealTimeDeparturesOpened, setAreRealTimeDeparturesOpened] = useState<boolean>(false);
    useEffect(() => {
        setLoading(true);
        intervalsRef.current.forEach(interval => clearInterval(interval));
        intervalsRef.current = [];

        service.getStopTimetable(Number((id ? id : stopId))).then((data) => {
            if (data) {
    ...
    const FormatDepartureTime = (data: StopTimetable[]) => {
        ...
        data.forEach(departure => {
            ...
            if (now > time) i++;
                ...
                if (time.getTime() - now.getTime() < 35*60000) departure.departure_text = "za " + Math.floor((time.getTime() - now.getTime())/60000) + " min";
                if (Math.floor((time.getTime() - now.getTime())/60000) < 1) departure.departure_text = ">>>";
            }
    ...
    return (
        <div className="MiniRealTimeDepartures">
            ...
            {loading ? <div>Loading...</div> : (
                <>
                <button onClick={() => setAreRealTimeDeparturesOpened(!areRealTimeDeparturesOpened)}><img width="20px" src={tabletIcon} />Rzeczywiste godziny odjazdów</button>
            {areRealTimeDeparturesOpened ? (
                <div className="realTimeDepartures"><div>
                {departures.slice(0, (departures.length > 6 ? 6 : departures.length)).map((departure, index) => (
                    <div key={index}>
                        <div>{departure.line_name}</div>
                        <div>{departure.last_stop_name}</div>
                        <div>{departure.departure_text ? departure.departure_text : departure.departure_time}</div>
                    </div>
                ))}
                <Link to={`/tablica/${(id ? id : stopId)}`}><img width="20px" src={fullscreenIcon} /></Link>
            </div></div>) : (<></>)
            }
