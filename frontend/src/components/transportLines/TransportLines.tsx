                    ...
                    <li key={lineType}>
                        ...
                        {Object.keys(transportLines[lineType]).map((lineName : string) => (
                            <div className="line-container" key={lineName}>
                                <div>{lineName}</div>
                                <div>
                                <span className="finalStops">{transportLines[lineType][lineName].at(0)} ↔ {transportLines[lineType][lineName].at(-1)}</span>
                                    {transportLines[lineType][lineName].map((stopName) => (
                                        <span key={stopName}>{stopName}{transportLines[lineType][lineName].at(-1) != stopName ? ' - ' : ''} </span>
                                    ))}
                                </div>
                            </div>
                        ))}
