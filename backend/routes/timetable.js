...
router.get('/departure-times', (req, res) => {
    ...
    const query = `
        SELECT r.id, r.line_id, r.stop_id, r.stop_number, r.route_number, r.travel_time, r.is_on_request,
               ts.stop_name, ts.stop_direction,
               tl.line_name, lt.line_type_name, lt.line_type_color, lt.line_type_image,
               tt.route_number, tt.departure_time, tt.id AS departure_id,
               ...
               (SELECT JSON_QUERY((SELECT tl2.line_name, lt2.line_type_color, r4.route_number
                ...
                AND tl2.id != r.line_id
                FOR JSON PATH))) AS other_lines,
               (SELECT JSON_QUERY((SELECT ts3.stop_name, r6.route_number, r6.stop_number, r6.travel_time, r6.is_on_request, r6.stop_id
                FROM routes r6
                JOIN transport_stops ts3 ON r6.stop_id = ts3.id
                WHERE r6.route_number = r.route_number
                FOR JSON PATH))) AS stops
        FROM timetable tt
        JOIN routes r ON tt.route_number = r.route_number
    ...
    sql.query(config.CONNECTION_STRING, query, (err, results) => {
        ...
        parsedResults.forEach(result => 
            result.departure_time = addMinutesToTime(result.departure_time, result.total_travel_time));
        ...
        let finalResult = {
            departure_times: parsedResults.map(result => ({ departure_time: result.departure_time, departure_id: result.departure_id })),
    ...
    sql.query(config.CONNECTION_STRING, query, (err, results) => {
        ...
        const promises = results.map((result) => {
            return new Promise((resolve, reject) => {
                ...
                sql.query(config.CONNECTION_STRING, travelTimeQuery, (err, results2) => {
                    if (err) {
                        console.error('Error running query:', err);
                        return reject('Error running query.');
                    }
                    result.departure_time = addMinutesToTime(result.departure_time, results2[0].total_travel_time);
                    resolve(result);
                });
            });
        });
                ...
                return new Promise((resolve, reject) => {
                    sql.query(config.CONNECTION_STRING, query, (err, results2) => {
                        ...
                        const responseArray = results2.map(doris => ({
                            ...doris,
                            departure_time: addMinutesToTime(doris.departure_time, doris.total_travel_time)
                        }));
